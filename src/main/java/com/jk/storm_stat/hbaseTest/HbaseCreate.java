...
public class HbaseCreate {
    ...
    public static void main(String[] args) {
        try {
            ...
            HbaseCreate.createTable(tableName, columnFamilys);
            // 第二步：向数据表的添加数据
            // 添加第一行数据
            if (isExist(tableName)) {
                HbaseCreate.addRow(tableName, "zpc", "info", "age", "20");
                HbaseCreate.addRow(tableName, "zpc", "info", "sex", "boy");
                HbaseCreate.addRow(tableName, "zpc", "course", "china", "97");
                HbaseCreate.addRow(tableName, "zpc", "course", "math", "128");
                HbaseCreate.addRow(tableName, "zpc", "course", "english", "85");
                // 添加第二行数据
                HbaseCreate.addRow(tableName, "henjun", "info", "age", "19");
                HbaseCreate.addRow(tableName, "<span style="+
                ">henjun</span>", "info", "sex", "boy");
                HbaseCreate.addRow(tableName, "henjun", "course", "china", "90");
                HbaseCreate.addRow(tableName, "henjun", "course", "math", "120");
                HbaseCreate.addRow(tableName, "henjun", "course", "english", "90");
                // 添加第三行数据
                HbaseCreate.addRow(tableName, "niaopeng", "info", "age", "18");
                HbaseCreate.addRow(tableName, "<span style="+
                ">niaopeng</span>", "info", "sex", "girl");
                HbaseCreate.addRow(tableName, "niaopeng", "course", "china", "100");
                HbaseCreate.addRow(tableName, "niaopeng", "course", "math", "100");
                HbaseCreate.addRow(tableName, "niaopeng", "course", "english", "99");
                // 第三步：获取一条数据
                System.out.println("**************获取一条(zpc)数据*************");
                HbaseCreate.getRow(tableName, "zpc");
                // 第四步：获取所有数据
                System.out.println("**************获取所有数据***************");
                HbaseCreate.getAllRows(tableName);

                // 第五步：删除一条数据
                System.out.println("************删除一条(zpc)数据************");
                HbaseCreate.delRow(tableName, "zpc");
                HbaseCreate.getAllRows(tableName);
                // 第六步：删除多条数据
                System.out.println("**************删除多条数据***************");
                String rows[] = new String[]{"qingqing", "xiaoxue"};
                HbaseCreate.delMultiRows(tableName, rows);
                HbaseCreate.getAllRows(tableName);
                // 第七步：删除数据库
                System.out.println("***************删除数据库表**************");
                HbaseCreate.deleteTable(tableName);
