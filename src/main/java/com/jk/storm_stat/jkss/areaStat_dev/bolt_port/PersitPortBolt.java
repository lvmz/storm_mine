 ...
 * @author shenfl
 */
public class PersitPortBolt extends BaseRichBolt {
    ...
    public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
        ...
        try {
            ...
            this.psQ = this.connection.prepareStatement("select count(1) from jike_calculate_day where useend = ?  and to_days(createtime) = to_days(now())");
            this.psI = this.connection.prepareStatement("insert into jike_calculate_day(userview, pageview, independip, useend, createtime) values(?,?,?,?,now())");
            this.psU = this.connection.prepareStatement("update jike_calculate_day set createtime = now(), userview = ?, pageview = ?, independip = ? WHERE useend = ?  and to_days(createtime) = to_days(now())");
...
//                System.out.println("tick end :" + new Timestamp(System.currentTimeMillis()));
            ...
            try {
                ...
                // 汇总各个访客端对应的 UV 数
                String key_uv = day  + "_" + useend + "_portuv";
                Long count_uv = jedis.scard(key_uv);
                // 汇总各个访客端对应的 PV 数
                String key_pv = day  + "_" + useend + "_portpv";
                Long count_pv = Long.parseLong(jedis.get(key_pv));
                // 汇总各个访客端对应的 独立IP 数
                String key_ip = day  + "_" + useend + "_portip";
